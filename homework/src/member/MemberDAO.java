...
public class MemberDAO {
	...
	public List<MemberDTO> list(Map map){
		...
		rs = pstmt.executeQuery();  // rs는 while 문 쓰는게 좋음
		while(rs.next()){
	...
	public boolean duplicateEmail(String email){
		...
		sql.append("   FROM member      ");
		sql.append("   WHERE email= ?      "); //값이 들어가야 된다

		try {
		
			pstmt = con.prepareStatement(sql.toString());
			pstmt.setString(1, email); //순번  , 벨류
	
			rs = pstmt.executeQuery();// 쿼리
			if(rs.next()){
				int cnt = rs.getInt(1);
			if(cnt>0) flag = true; // 중복된 아이디 이다.
			}
	...
	public boolean duplicateId(String id){
		...
		sql.append("   FROM member      ");
		sql.append("   WHERE id= ?      "); //값이 들어가야 된다

		try {
		
			pstmt = con.prepareStatement(sql.toString());
			pstmt.setString(1, id); //순번  , 벨류
