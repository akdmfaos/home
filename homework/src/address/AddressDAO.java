...
public class AddressDAO {
	...
	public boolean create(AddressDTO dto){
		...
		sql.append(" values((select nvl(max(no),0)+1 as no from address), ");
		sql.append(" ?, ?, ?, ?, ?, sysdate) ");
	...
	public AddressDTO read (int no){
		...
		ResultSet rs = null;
		StringBuffer sql = new StringBuffer();
		sql.append(" select no ,name , phone, zipcode, address1, address2, to_char(wdate, 'yyyy-mm-dd') wdate from address ");
		sql.append(" where no=? ");
		try {
			pstmt=con.prepareStatement(sql.toString());
			pstmt.setInt(1, no);
	...
	public boolean update(AddressDTO dto){
		...
		PreparedStatement pstmt = null;
		StringBuffer sql = new StringBuffer();
		sql.append(" update address set phone=?, zipcode=?, address1=?, address2=? ");
		sql.append(" where no=? ");
		try {
			pstmt=con.prepareStatement(sql.toString());
